...
const textToArray = (text) => {
  let cleanText = text.toLowerCase().replace(/[^A-Za-z0-9ÁáÉéÍíÓóÚúÑñ]+/g, " ");
  let arrayFromText = cleanText.trim().split(" ");
  return arrayFromText;
};

const setValue = (key, map, array, value) => {
  if(!map.has(key)){
    value = 1;
    return value;
  } else {
...
const wordFrequency = (text) => {
  ...
  for (let i = 0; i < textArray.length; i++) {
    wordMap.set(textArray[i], setValue(textArray[i], wordMap, textArray, value));
  }
  var wordFrequencyAsc = new Map([...wordMap.entries()].sort((a, b) => b[1] - a[1]));
  return wordFrequencyAsc
};
